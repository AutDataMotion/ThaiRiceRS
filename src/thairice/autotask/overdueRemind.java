...
public class overdueRemind extends Controller implements ITask {
	...
	public void run() {
		...
		for (T3user t3user : list) {
			  long day=TimeUtil.dateDiff(t3user.getTimestamp("Prdt_EfDt").toString(), t3user.getTimestamp("PD_ExDat").toString(), "yyyy-MM-dd HH:mm:ss", "d");
			 //距离服务到期还有7天提醒用户续费
			  if(day<=7){
				  //避免重复发送
				  T8message hasCheck=T8message.dao.findFirst("select * from t8message where send_userid=? and flag=3",t3user.getBigInteger("id"));
				  if(hasCheck==null){
