...
public class overdueRemind extends Controller implements ITask {
	...
	public void run() {
		...
		for (T3user t3user : list) {
			...
			if (day <= 7) {
				// 避免重复发送
				T8message hasCheck = T8message.dao.findFirst("select * from t8message where send_userid=? and flag=3",
						t3user.getBigInteger("id"));
				if (hasCheck == null) {
					T8message t8message = new T8message();
					t8message.set("flag", 3);
					t8message.setContent(
							"你的产品服务准备到期了,到期时间:" + t3user.getTimestamp("PD_ExDat").toString() + ",为了不影响你的使用,请尽快续费。");
