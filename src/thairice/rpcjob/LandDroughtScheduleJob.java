...
public class LandDroughtScheduleJob extends AbsScheduleJob implements ITask {
	...
	public List<T12PreProcessInf> loadDataFromDb(){
		...
		return T12PreProcessInf.dao.find( String.format(" select * from %s where   %s and data_type =%s   limit 100 "
				...
				, whereStr
				, EnumDataStatus.DATA_TYPE_NDVI_1.getIdStr()) );
	}
	...
	public List<Drought> mdlConvert(List<T12PreProcessInf> inputs ){
		...
		List<T12PreProcessInf> dataLSTs = T12PreProcessInf.dao.find( String.format(" select * from %s where   %s and data_type =%s   limit 100 "
				, T12PreProcessInf.tableName
				, whereStr
				, EnumDataStatus.DATA_TYPE_LST.getIdStr()) );
		...
		T12PreProcessInf ndviObj = inputs.get(idx_ndvi);
		T12PreProcessInf ndviObjNext = null;
		if (idx_ndvi_next < inputs.size()) {
			ndviObjNext = inputs.get(idx_ndvi_next);
		} 
...
//					if (lstCollectTime.compareToIgnoreCase(ndviObjNext.get)) {
//						
//					}
				}else {
		...
		List<Drought> resList  = inputs.stream().map(preObj -> {
			Drought target = new Drought();
			// 用该数据的id作为taskID
			target.id = preObj.getId();
			target.fileDate = GenerTimeStamp.pickDateStr(preObj.getData_collect_time());
			target.imageLst = (String)preObj.getFile_path() + (String)preObj.getFile_name();
			target.imageNdvi = preObj.getFile_name();
	...
	private void initParams(){
		List<Record> params = ConfMain.db().find("SELECT value_ FROM t1parameter where parm_type_id = '10000003' order by parm__id ");
